            ...
            <header className="w-full bg-white/80 backdrop-blur-lg border-b border-[var(--color-border)] sticky top-0 z-30">
                <div className="w-full max-w-7xl mx-auto flex justify-between items-center p-4">
                    <div className="flex items-center gap-2">
                        <svg className="w-8 h-8 text-blue-600" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
                        <h1 className="text-xl font-bold tracking-tight text-slate-900">Portal CEJA</h1>
                    </div>
                    <div id="user-controls">
                        {!profile ? (
                            <button onClick={() => setLoginModalOpen(true)} className="bg-blue-600 text-white font-semibold py-2 px-5 rounded-lg hover:bg-blue-700 transition text-sm shadow-sm hover:shadow-md">Login</button>
                        ) : (
                                ...
                                {isProfileMenuOpen && (
                                    <div className="absolute top-full right-0 mt-2 w-72 bg-white rounded-xl shadow-lg border border-slate-200/80 p-2 origin-top-right animate-fadeIn" style={{ animationDuration: '200ms' }}>
                                        <div className="p-3 text-center border-b border-slate-100">
                                            <p className="font-bold text-slate-800 text-base truncate" title={profile.nome_completo || profile.email}>
                                        ...
                                        <div className="py-2">
                                            ...
                                            <button onClick={handleLogout} disabled={isLoggingOut} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-red-600 rounded-lg hover:bg-red-50 transition">
                                                {isLoggingOut ? (
                                                    <div className="spinner" style={{ width: '20px', height: '20px', borderWidth: '2px', borderLeftColor: 'currentColor' }}></div>
                                                ) : (
...
const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const getProfile = useCallback(async (session: AuthSession) => {
    if (!session?.user) {
        setProfile(null);
        return;
    }
...
function App() {
  const [currentView, setCurrentView] = useState('welcome'); // welcome, student_dashboard, quiz_taker
  ...
  return (
    <AuthProvider>
      <AppContent currentView={currentView} setCurrentView={setCurrentView} startExam={startExam} activeExamId={activeExamId} endExam={endExam} />
    </AuthProvider>
  );
}
...
const AppContent: React.FC<AppContentProps> = ({ currentView, setCurrentView, startExam, activeExamId, endExam }) => {
    ...
    }, [profile, loading, currentView, setCurrentView]);
    ...
    const renderContent = () => {
        ...
        switch (currentView) {
            case 'quiz_taker':
                return activeExamId ? <QuizTaker examId={activeExamId} onFinish={endExam} /> : <p>Erro: Nenhuma avaliação selecionada.</p>;
            case 'student_dashboard':
                return profile ? <StudentView onStartExam={startExam} /> : null;
            case 'welcome':
            default:
                return (
