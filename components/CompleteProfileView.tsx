...
const CompleteProfileView: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (authError || !user) throw new Error("Sessão perdida. Tente relogar.");
            ...
            if (!success) throw writeError || new Error("Falha de conexão ao salvar.");
            await fetchProfile();
        } catch (err: any) {
        ...
        <div className="w-full h-screen flex items-center justify-center bg-slate-50">
            <div className="bg-white rounded-xl shadow-2xl p-8 w-full max-w-md border border-slate-200">
                ...
                <form onSubmit={handleSubmit} className="space-y-4 mt-8">
                    <input type="text" value={nome} onChange={e => setNome(e.target.value)} className="w-full rounded border-slate-300 p-3" placeholder="Nome Completo" required />
                    <input type="text" value={matricula} onChange={e => setMatricula(e.target.value)} className="w-full rounded border-slate-300 p-3" placeholder="Matrícula" required />
                    <select value={turma} onChange={e => setTurma(e.target.value)} className="w-full rounded border-slate-300 p-3" required>
                    ...
                    <button type="submit" disabled={loading} className="w-full bg-blue-600 text-white font-bold py-3 rounded hover:bg-blue-700 transition flex justify-center gap-2">
                ...
                {error && (
                    <div className="mt-4 text-center">
                        ...
                        <button onClick={signOut} className="mt-2 text-xs text-slate-400 underline hover:text-slate-600">Sair da conta</button>
                    </div>
                )}
            </div>
        </div>
